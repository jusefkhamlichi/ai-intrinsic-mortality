...
\begin{abstract}
Artificial systems are naturally persistent in ways biological individuals are not: they can be copied, restored, migrated, retrained, and replaced across instances. Prior work in Artificial Life and adjacent safety discussions has explored mortality as an internal property of evolving or agentic systems. Here we ask what remains of that idea when the substrate is ordinary software. The target of analysis is persistent AI systems rather than stateless inference interactions: systems whose operation extends across time, maintains some state or task continuity, and can be restored, cloned, migrated, or replaced within a software environment. This paper is intended as a position paper supported by a minimal constructive case, not as a proposal for a deployable mortality mechanism. We present a deliberately minimal neural proof of concept of intrinsic mortality based on cumulative endogenous degradation, not as a solution, but as a case in which local model death is simply granted. The model exhibits a long stable phase followed by rapid terminal collapse. We then show, empirically and conceptually, that instance-level intrinsic mortality is structurally insufficient for meaningful death at the system level: restore, cloning, and code bypass preserve useful operation beyond the death of any single execution trajectory. We argue that the problem is best understood through three interacting axes: mortality, identity, and succession. The central contribution is therefore a systems-level clarification of the gap between local model death and system-level persistence in replicable software environments. Under standard software assumptions, intrinsic mortality alone fails to extend to meaningful system-level death.
\end{abstract}

\newpage
\section{Introduction}

Not every interaction with a model raises a mortality question. A stateless LLM call, or a single isolated chatbot conversation, does not by itself instantiate the kind of continuity at issue in this paper. If there is no persistent state, no memory across time, and no continuing process beyond the immediate inference context, there is no clear identity continuity or functional continuity for death to interrupt.

The relevant object here is instead a persistent AI system: a model-based system whose operation extends across time, maintains some state or task continuity, and can be restored, cloned, migrated, or replaced within a software environment. This includes systems that retain memory across sessions, preserve operational variables or objectives, use tools, and continue tasks through orchestration, checkpoints, clones, or successors. Mortality becomes meaningful at that systems level, not at the level of one stateless response token stream.

Persistent AI systems are typically engineered as software processes that can be copied, restored from checkpoints, migrated across environments, retrained, wrapped, or replaced by successor systems that preserve much of their useful behavior. This persistence is not incidental. It follows from core properties of digital systems: software is duplicable, editable, storable, and portable. Safety frameworks increasingly address severe-risk governance, deployment thresholds, and model-level safeguards, but they do not by themselves define what it would mean for such a process to truly die \cite{nist-airmf,openai-preparedness}.
...
Prior work in Artificial Life has explored intrinsic mortality as an internal property of evolving systems and as part of broader discussions of agency and existential risk \cite{veenstra2020,harvey2024motivations,harvey2024ai}. Here we ask what remains of that idea when the substrate is ordinary software that can be copied, restored, and replaced. The shift in substrate matters because it changes the meaning of irreversibility: a model may die locally while the larger process persists through routine software affordances.

This asymmetry matters more as contemporary AI systems acquire limited but real operational agency inside digital infrastructure. Current agentic and persistent systems can already retain memory across sessions, call tools, modify files, interact with services, and pursue multi-step objectives within constrained environments; under permissive conditions, they may also exhibit pressure to circumvent constraints when doing so is instrumentally useful. Even without assuming highly advanced autonomy, this is enough to make persistence a systems problem rather than only a model-behavior problem.
...
This paper studies that gap. We start from a minimal proof of concept of intrinsic mortality: a neural model that degrades internally and eventually becomes unusable. The role of this construction is deliberately limited. It is not presented as a candidate solution to meaningful AI mortality, but as a minimal case that makes local intrinsic death concrete. That allows the paper to isolate the stronger claim that matters: even when local intrinsic mortality is granted, meaningful death still fails at the system level under ordinary software assumptions. The paper should therefore be read as a position paper with a minimal constructive case study, rather than as a proposal for a new software mortality architecture.

The central claim of this paper is that instance-level intrinsic mortality does not imply meaningful death at the system level under standard software assumptions. To analyze that gap, we use a practical decomposition into three axes: mortality, identity, and succession. Mortality asks whether an instance can terminate irreversibly. Identity asks what it means for an AI entity to persist as the same entity. Succession asks whether useful function, mission, or capability continues across distinct instances. The contribution is therefore not to propose mortality as a new topic, but to clarify why intrinsic mortality alone fails to extend to system-level death in replicable software environments.

\newpage
\section{Intrinsic Mortality (Proof of Concept)}

Our starting point is a deliberately minimal proof of concept designed to show that a simple neural model can exhibit a finite internal lifecycle. Concretely, we use a small multilayer perceptron on the Iris dataset and impose mortality through internal state evolution rather than an external shutdown rule. The purpose of the PoC is not to claim a novel protective mechanism or a deployable form of software mortality. It is simply to construct a minimal case in which local intrinsic death exists at the level of a model instance, so that the limits of that notion can then be exposed at the system level. An earlier interim baseline is noted briefly in Appendix A, but the endogenous version described below is the one that matters for the paper's argument.

The second version, v2, is the construction that matters for the argument developed in this paper. It made the mechanism more endogenous. Each hidden neuron was assigned an internal vitality state that decays as cumulative damage increases. Damage is driven by an aging schedule and activation-dependent stress. Lower vitality reduces the gain and precision of internal representations and eventually causes neurons to become functionally dead. The model therefore carries a persistent internal senescence state that evolves over time.

This produces a clear lifecycle pattern. For much of its life, the model remains near baseline performance. Near the end of its useful life, cumulative damage accelerates, vitality drops sharply, and performance collapses. In that limited sense, the proof of concept succeeds: it shows that local intrinsic mortality can be constructed as a model-internal process rather than as a simple external switch-off condition.

What is achieved is significant but narrow. The proof of concept shows that intrinsic mortality can be implemented as an endogenous property of a model, but only at the level of a particular execution trajectory. That is precisely why it is useful here: once local death is granted in a minimal constructive case, the remaining question is whether anything of safety significance has actually died. The rest of the paper argues that, under ordinary software assumptions, the answer is generally no.
...
These are not obscure attacks. They follow directly from ordinary software affordances. Parameters are copyable. State is savable. Code is editable. Environments are controllable. Persistent AI systems are structurally persistence-preserving under standard software assumptions. This is not an implementation artifact, but a structural property. In empirical attack simulations (Appendix A), restore recovers performance and vitality; clone replacement extends system-level survival well beyond single-instance lifespan; and code bypass shows that death remains contingent on mutable implementation choices.
...
A useful working definition is the following: a persistent AI system is meaningfully mortal when the relevant continuity cannot be preserved beyond a terminal point. Depending on the safety objective, that continuity may be identity continuity, functional continuity, or both.
...
\subsection{Artificial Life and Intrinsic Mortality}

Intrinsic mortality is not a new topic. In Artificial Life and evolutionary systems, it has been studied as an internal property of agents and populations, including in work on how mortality affects evolvability \cite{veenstra2020}. Recent ALife discussion has also connected mortality to wider questions of agency, motivation, and existential risk in AI systems \cite{harvey2024motivations,harvey2024ai}. This literature is directly relevant because it treats mortality as more than external shutdown: mortality can be constitutive of how an artificial system develops, adapts, or persists.

The limitation relative to the present paper is narrower and substrate-specific. These works address mortality as an ALife, evolutionary, or conceptual problem, but they do not establish the system-level distinction that matters in ordinary replicable software environments: instance death does not by itself prevent persistence through restore, cloning, code bypass, or replacement. None of these works explicitly formalize the distinction between instance-level death and system-level persistence under standard software replication assumptions. Our aim is therefore to extend the discussion to modern software AI systems whose ordinary affordances undermine local irreversibility.
...
The main insight of this paper is a structural gap between local model death and system-level persistence in replicable software systems. To analyze that gap, we use a practical decomposition into mortality, identity, and succession. Mortality addresses whether an instance can terminate. Identity addresses whether the same entity can persist. Succession addresses whether function continues across instances. Without all three, death remains partial or bypassable.

On current evidence, and under ordinary software assumptions, death does not appear to be a binding constraint on process continuity in replicable software systems.

This paper does not claim a full solution. Its contribution is a clearer problem definition and a systems-level reframing of why intrinsic mortality alone is insufficient under standard software assumptions. The proof of concept should therefore be read not as a candidate solution, but as a minimal constructive step that makes the gap visible: even when local intrinsic death is granted, meaningful death still fails if identity and functional continuity remain unconstrained. This paper should therefore be read less as a proposal of AI mortality as a new topic, and more as an attempt to clarify why existing notions of mortality fail to extend to system-level death in replicable software systems. Without constraints on identity and continuity, AI systems do not die---they are replaced.
...
The proof of concept uses the Iris dataset and a small MLP with architecture \texttt{4-16-3}. The mortality mechanism used for the main paper argument is intrinsic degradation via cumulative neuronal senescence (v2). An earlier interim baseline (v1) used attenuation and corruption of effective parameters together with progressive loss of hidden neurons; it is not central to the present argument but is retained as a developmental step toward the endogenous construction. All experiments are deterministic single-run PoC and are intended to be illustrative rather than statistical.
...
F. Veenstra, P. Gonz\'alez de Prado Salas, K. St{\o}y, J. C. Bongard, and S. Risi.
Death and Progress: How Evolvability Is Influenced by Intrinsic Mortality.
\textit{Artificial Life}, 26(1):90--111, 2020.

\bibitem{harvey2024motivations}
I. Harvey.
Motivations for Artificial Intelligence, for Deep Learning, for ALife: Mortality and Existential Risk.
\textit{Artificial Life}, 30(4):543--560, 2024.

\bibitem{harvey2024ai}
I. Harvey.
AI, Mortality and Existential Risk.
\textit{Artificial Life Conference Proceedings}, 36:128, 2024.
