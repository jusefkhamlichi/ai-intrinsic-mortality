...
\begin{abstract}
Artificial systems are naturally persistent in ways biological individuals are not: they can be copied, restored, migrated, retrained, and replaced across instances. Prior work in Artificial Life and adjacent safety discussions has explored mortality as an internal property of evolving or agentic systems \cite{veenstra2020,harvey2024motivations,harvey2024ai}. Here we examine what remains of that idea when the relevant substrate is ordinary software. We present a neural proof of concept of intrinsic mortality based on cumulative endogenous degradation, and show that the resulting model exhibits a long stable phase followed by rapid terminal collapse. We then validate, empirically and conceptually, that instance-level intrinsic mortality is insufficient for meaningful death at the system level: restore, cloning, and code bypass preserve useful operation beyond the death of any single execution trajectory. We argue that the problem is best decomposed into three interacting axes: mortality, identity, and succession. The central contribution is therefore not mortality as a new topic, but a systems-level clarification of the gap between local model death and system-level persistence in replicable software environments. Under standard software assumptions, intrinsic mortality alone does not straightforwardly extend to meaningful system-level death.
\end{abstract}
...
Prior work in Artificial Life has explored intrinsic mortality as an internal property of evolving systems and as part of broader discussions of agency and existential risk \cite{veenstra2020,harvey2024motivations,harvey2024ai}. Here we ask what remains of that idea when the relevant substrate is ordinary software that can be copied, restored, and replaced. The shift in substrate matters because it changes the meaning of irreversibility: a model may die locally while the larger process persists through routine software affordances.
...
The central claim of this paper is that instance-level intrinsic mortality does not imply meaningful death at the system level under standard software assumptions. To analyze that gap, we use a practical decomposition into three axes: mortality, identity, and succession. Mortality asks whether an instance can terminate irreversibly. Identity asks what it means for an AI entity to persist as the same entity. Succession asks whether useful function, mission, or capability continues across distinct instances. The contribution is therefore not to propose mortality as a new topic, but to clarify why intrinsic mortality alone does not straightforwardly extend to system-level death in replicable software environments.
...
The limitation relative to the present paper is narrower and substrate-specific. These works address mortality as an ALife, evolutionary, or conceptual problem, but they do not establish the system-level distinction that matters in ordinary replicable software environments: instance death does not by itself prevent persistence through restore, cloning, code bypass, or replacement. Our aim is therefore not to displace that literature, but to extend the discussion to the case of modern software AI systems whose ordinary affordances undermine local irreversibility.
...
In replicable software systems, death is not a binding constraint on process continuity.

This paper does not claim a full solution. Its contribution is a clearer problem definition and a systems-level reframing of why intrinsic mortality alone is insufficient under standard software assumptions. If AI systems are to be meaningfully mortal, the challenge is not merely how to kill a model, but how to prevent persistence of the relevant entity or process. This paper should therefore be read less as a proposal of AI mortality as a new topic, and more as an attempt to clarify why existing notions of mortality do not straightforwardly extend to system-level death in replicable software systems. Without constraints on identity and continuity, AI systems do not die---they are replaced.
